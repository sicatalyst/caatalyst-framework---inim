...
### ACF Field Retrieval Pattern

**Category:** Data Flow
**Status:** Active
**Description:** All modules retrieve ACF data using get_field() at the top of the file, then conditionally render based on field existence.
**Implementation:**
```php
<?php
...
<?php if ($field_name): ?>
  <div><?php echo esc_html($field_name); ?></div>
<?php endif; ?>
...
<?php endif; ?>
```
**Rationale:** Prevents errors when fields are empty. Separates data retrieval from rendering logic. Ensures graceful degradation.

### Output Escaping Pattern

**Category:** Security
**Status:** Active
**Description:** All dynamic output must be escaped based on context: esc_html() for text, esc_url() for URLs, esc_attr() for attributes, wp_kses_post() for rich text.
**Implementation:**
```php
...
```
**Rationale:** Prevents XSS attacks. WordPress coding standards requirement. Different contexts require different escaping functions.
